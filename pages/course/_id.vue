                      ...
                      <menu id="lh-menu" class="lh-menu mt10 mr10">
                        <ul>
                          ...
                          <li class="lh-menu-stair" v-for="chapter in chapterVideoList" :key="chapter.id">
                            ...
                            <ol class="lh-menu-ol" style="display: block;" v-if="isbuyCourse || Number(courseWebVo.price)===0">
                              <li class="lh-menu-second ml30" v-for="video in chapter.children" :key="video.id">
                                <a :href="'/player/'+video.videoSourceId" target="_blank">
                                  <span class="fr">
                                    <i class="free-icon vam mr10">敬请观看</i>
                                  </span>
                            ...
                            <ol class="lh-menu-ol" style="display: block;" v-else>
                              <li class="lh-menu-second ml30" v-for="video in chapter.children" :key="video.id">
                                <a  target="_blank" @click="goToPay()">
                                  <span class="fr">
                                    <i class="free-icon vam mr10">付费观看</i>
                                  </span>
                                  <em class="lh-menu-i-2 icon16 mr5">&nbsp;</em>{{video.title}}
                                </a>
                              </li>                              
...
export default {
  ...
  //    return course.getById(params.id)
  //       .then(response => {
  //         return {
  ...
  methods:{
    //获取课程详情
    initCourseInfo() {
      course.getById(this.comment.courseId)
            .then(response => {
    ...
    initComment(){
       comment.getPageList(this.page, this.limit, this.comment.courseId).then(response => {
           this.data = response.data.data
           //console.log(this.data)
       })
    },
    addComment(){
        this.comment.courseId = this.comment.courseId
    ...
    gotoPage(page){
          comment.getPageList(page, this.limit,this.comment.courseId).then(response => {
              this.data = response.data.data
          })
    },
    //根据课程id，调用接口方法生成订单
    createOrder(){
        order.createOrder(this.comment.courseId).then(response => {
            if(response.data.success){
              //订单创建成功，跳转到订单页面
